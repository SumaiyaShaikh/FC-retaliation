...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\sumsh86\\OneDrive - Linköpings universitet\\New Psychopy aggression\\AggPrototypeRetaliationTest.py',
...
for thisRetaliationLoop in RetaliationLoop:
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if t >= 0.0 and Shock0.status == NOT_STARTED:
            # keep track of start time/frame for later
            Shock0.tStart = t  # not accounting for scr refresh
            Shock0.frameNStart = frameN  # exact frame index
            win.timeOnFlip(Shock0, 'tStartRefresh')  # time at next scr refresh
        ...
        if Shock0.status == STARTED and t >= frameRemains:
            # keep track of stop time/frame for later
            Shock0.tStop = t  # not accounting for scr refresh
            Shock0.frameNStop = frameN  # exact frame index
            win.timeOnFlip(Shock0, 'tStopRefresh')  # time at next scr refresh
            Shock0.setAutoDraw(False)
        
        # *Shock1* updates
        if t >= 0.0 and Shock1.status == NOT_STARTED:
            # keep track of start time/frame for later
            Shock1.tStart = t  # not accounting for scr refresh
        ...
        if Shock1.status == STARTED and t >= frameRemains:
            ...
            win.timeOnFlip(Shock1, 'tStopRefresh')  # time at next scr refresh
            Shock1.setAutoDraw(False)
        
        # *Shock2* updates
        if t >= 0.0 and Shock2.status == NOT_STARTED:
            # keep track of start time/frame for later
            Shock2.tStart = t  # not accounting for scr refresh
        ...
        if Shock2.status == STARTED and t >= frameRemains:
            ...
            win.timeOnFlip(Shock2, 'tStopRefresh')  # time at next scr refresh
    ...
    RetaliationLoop.addData('Shock0.started', Shock0.tStartRefresh)
    RetaliationLoop.addData('Shock0.stopped', Shock0.tStopRefresh)
    RetaliationLoop.addData('Shock1.started', Shock1.tStartRefresh)
    RetaliationLoop.addData('Shock1.stopped', Shock1.tStopRefresh)
    RetaliationLoop.addData('Shock2.started', Shock2.tStartRefresh)
    RetaliationLoop.addData('Shock2.stopped', Shock2.tStopRefresh)
    thisExp.nextEntry()
